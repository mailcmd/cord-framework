...
const CORD = function() {
    ...
    const proxy_handler = {
        get(target, field, receiver) {
            ...
            } else if (field_name in window) {
                return window[field_name]
            } else {
    ...
    const ifsParser = function(str) {
        ...
        const extractIfParts = function(str) {
            const [[_, exp, xattrs, body, elsebody]] = str
                  .matchAll(/[\t ]+(.+?)(?:|[ ]+:xattrs\[(.+?)\][ ]*?):do(.+?)(?::else(.+?)|):endif/sg)
                  .toArray();
            return [[':if' + str, exp, xattrs, body, elsebody]];
        };

        const replaceIf = function(str, matches) {
            const replaces = matches
                  .map( ([_, exp, xattrs, body, elsebody]) => {
                      return `
                  <template if="\$\{${exp}\}" xattrs="${xattrs}">
                    ${body}
                  </template>
                  `+(elsebody ? `
                  <template if="\$\{!(${exp})\}" xattrs="${xattrs}">
            ...
            return matches
                .reduce((acc, [str, _x, _a, _b, _c], i) => acc.replace(str, replaces[i]), str);
        };
    ...
    const process_container_script = function(tpl, map, cord_id) {
        ...
        tmp.querySelectorAll('cord-script').forEach( cs => {
            ...
            tpl.innerHTML = tpl.innerHTML.replace(/<cord-script>(?:.+?)<\/cord-script>/s, '')
        ...
        tmp.querySelectorAll('cord-script-after-render').forEach( cs => {
            ...
            tpl.innerHTML = tpl.innerHTML
                .replace(/<cord-script-after-render>(?:.+?)<\/cord-script-after-render>/s, '')
                // .replace(cs.innerHTML, '')
    ...
    const process_container_ifs = function(elem) {
        ...
        elem.querySelectorAll('template[if]').forEach(tpl => {
            tpl.cordContainer = cord_id;
            const xattrs = tpl.getAttribute('if')
                  ? tpl.getAttribute('if').split(',').map(a=>a.trim())
                  : [];
            [...xattrs, ...get_identifiers(tpl.getAttribute('if')+','+tpl.innerHTML, cord_id)]
              .forEach(f => {
    ...
    const render_container = function(cord_id, field) {
        ...
        if (elem.cordEvalAfterRender) {
            try {
                cord_eval(elem.cordEvalAfterRender, env, {as_string: false});
            } catch(e){
                console.warn('Error in cord-script-after-render tag content: ', e.message, env);
            }
        }
        ...
        [...cord_containers_affected].forEach(cid => {
            ...
            if (js) {
                try {
                    let env = {...DATAS[cid], ...{$self: DATAS[cid], $global: DATAS}}
                    cord_eval(elem.cordEvalAfterRender, env, {as_string: false});
                } catch(e){
